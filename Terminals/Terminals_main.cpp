		  ...
		  TGridDrawState State)
{
	if(ARow == 0 && ACol==0 && forsort !=0)
	{
		...
		Rect.Bottom-=3;
		StrTabl->Canvas->Brush->Color = clBtnFace;
...
{   bool zanat =false;
	...
	DTT->Canvas->TextRect(Rect,Rect.Left+9,Rect.Top+2,DTT->Cells[ACol][ARow]);
}
//---------------------------------------------------------------------------
...
if(Ch_autoW->Checked)
	{
		tElem width=StrTabl->Canvas->TextWidth(StrTabl->Cells[0][0])+10;
		for(int i=0;i<trmnlsPep.Length;i++)
		if(width< StrTabl->Canvas->TextWidth(String(trmnlsPep[i].Phio.c_str()))+10)
			width=StrTabl->Canvas->TextWidth(String(trmnlsPep[i].Phio.c_str()))+10;
		StrTabl->ColWidths[0] = width;
	}
else StrTabl->ColWidths[0]=StrTabl->Canvas->TextWidth(StrTabl->Cells[0][0])+10;
}
//---------------------------------------------------------------------------
